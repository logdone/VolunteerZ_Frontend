...
describe('Comment e2e test', () => {
  ...
  it('should load Comments', async () => {
    ...
    await element.all(by.css('ion-item'))
      .filter(async el => (await el.element(by.css('h2')).getText()) === 'Comment').first().click();

    commentComponentsPage = new CommentComponentsPage();
    await browser.wait(ec.visibilityOf(commentComponentsPage.title), 5000);
    expect(await commentComponentsPage.getTitle())
      .toEqual(COMPONENT_TITLE);
    await browser.wait(ec.or(ec.visibilityOf(commentComponentsPage.entities.get(0)), ec.visibilityOf(commentComponentsPage.noResult)), 5000);
  });
  ...
  it('should create Comment', async () => {
    ...
    expect(await commentUpdatePage.getPageTitle())
    ...
    expect(await commentComponentsPage.getTitle())
      .toEqual(COMPONENT_TITLE);
    expect(await commentComponentsPage.getEntitiesNumber())
  ...
  it('should view the last Comment', async () => {
    ...
    expect(await commentDetailPage.getPageTitle())
      .toEqual(SUBCOMPONENT_TITLE);
